...
abstract class GlyphShapingTableEntry extends TableEntry {
  ...
  void readContentFrom(ByteOrderSwappingBinaryReader reader) {
    ...
    int tableStartAt = reader.position;

    majorVersion = reader.readUInt16();
    minorVersion = reader.readUInt16();

    int scriptListOffset = reader.readUInt16(); // from beginning of table
    int featureListOffset = reader.readUInt16(); // from beginning of table
    int lookupListOffset = reader.readUInt16(); // from beginning of table
    int featureVariations = (minorVersion == 1)
        ? reader.readUInt32()
        : 0; // from beginning of table
    ...
    //2. feature list
    featureList =
        FeatureList.createFrom(reader, tableStartAt + featureListOffset);
    //-----------------------
    //3. lookup list
  ...
  void readLookupListTable(
      ByteOrderSwappingBinaryReader reader, int lookupListBeginAt) {
    ...
    reader.seek(lookupListBeginAt);
    int lookupCount = reader.readUInt16();
    List<int> lookupTableOffsets = Utils.readUInt16Array(reader, lookupCount);

    //----------------------------------------------
    //load each sub table
    //https://www.microsoft.com/typography/otspec/chapter2.htm
    for (int lookupTableOffset in lookupTableOffsets) {
      int lookupTablePos = lookupListBeginAt + lookupTableOffset;
      reader.seek(lookupTablePos);

      int lookupType = reader
          .readUInt16(); //Each Lookup table may contain only one type of information (LookupType)
      int lookupFlags = reader.readUInt16();
      ...
      List<int> subTableOffsets = Utils.readUInt16Array(reader, subTableCount);

      int markFilteringSet =
          ((lookupFlags & 0x0010) == 0x0010) ? reader.readUInt16() : 0;
